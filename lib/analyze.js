...
const SYNTHESIS_SYSTEM = `You are an expert book analyst. You have chapter-by-chapter summaries and cross-chapter context for a book. Return a JSON object with EXACTLY these fields:
...
The nodes and edges form a cross-book knowledge graph. Aim for 20-28 nodes with meaningful labeled edges.
Return ONLY valid JSON. No markdown, no explanation, no code fences.`;
...
export async function synthesize(chapters, chapterAnalyses, index, options = {}) {
  ...
  // Use only the first paragraph of each summary so all chapters fit regardless of book length.
  // Full summaries are already saved in analyses.json — synthesis only needs the distilled view.
  const chaptersBlock = chapters.map((ch, i) => {
    const r = chapterAnalyses[i] || {};
    const summaryFirst = (r.summary || '').split('\n').filter(Boolean)[0] || '';
